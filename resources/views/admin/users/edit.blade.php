            ...
            <div class="leading-loose">
                <form class="p-10 bg-white rounded shadow-xl" method="POST" action="{{ route('users.update' ,$user->id) }}" >
                    ...
                    <div class="mt-2">
                        ...
                        <input class="w-full px-5 py-4 text-gray-700 bg-gray-200 rounded" id="name" name="name" type="text" value="{{old('name',$user->name)}}"  placeholder="Your Name" aria-label="Name">
                        @error('name')
                    ...
                    <div class="mt-2">
                        ...
                        <input class="w-full px-5 py-4 text-gray-700 bg-gray-200 rounded" id="email" name="email" type="email" value="{{old('email' ,$user->email)}}"  placeholder="Your email" aria-label="email">
                        @error('email')
                        <p class="text-red-500 text-xs">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mt-2">
                        ...
                        <input class="w-full px-5 py-4 text-gray-700 bg-gray-200 rounded" id="password" name="password" type="password" disabled value="{{old('password',$user->password)}}"  placeholder="Your Password" aria-label="password">
                        @error('password')
                        <p class="text-red-500 text-xs">{{ $message }}</p>
                        @enderror
                    </div>
