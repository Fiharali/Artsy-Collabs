    ...
    <main class="w-full flex-grow p-6 mx-auto bg-amber-500">
        ...
        <div class="w-2/3 lg:w-1/2 my-6 pr-0 lg:pr-2 bg-amber-950 mx-auto">
            <div class="leading-loose">
                <form class="p-10 bg-white rounded shadow-xl" enctype="multipart/form-data"  method="POST" action="{{ route('projects.update',$project->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="mt-2">
                        ...
                        <input class="w-full px-5 py-4 text-gray-700 bg-gray-200 rounded" id="title" name="title"
                               type="text" value="{{old('title',$project->title)}}" placeholder="Your title" aria-label="title">
                        @error('title')
                    ...
                    <div class="mt-2">
                        ...
                        <textarea class="w-full px-5 py-4 text-gray-700 bg-gray-200 rounded" id="description" name="description"
                                  type="email" placeholder="Your description" aria-label="description"> {{old('description',$project->description)}}</textarea>
                        @error('description')
                        <p class="text-red-500 text-xs">{{ $message }}</p>
                        @enderror
                    </div>
